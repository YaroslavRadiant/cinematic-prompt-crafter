...
const isPhotographyGenre = (segment: string): boolean => {
  ...
  return (
    ...
    PHOTOGRAPHY_GENRES.some((genre) =>
      normalizedSegment.includes(`${genre.toLowerCase()} photography`)
    )
  );
};
...
const isPhotographyAesthetic = (segment: string): boolean => {
  return (
    ...
    PHOTOGRAPHY_AESTHETICS.some((aesthetic) =>
...
const parsePrompt = (promptText: string) => {
  ...
  segments.forEach((segment, index) => {
    ...
    if (isCameraAngle(segment)) {
      ...
      addParameter("Camera Angle", matchingAngle || segment, CAMERA_ANGLES);
    }
    ...
    if (isInspiredBy(segment)) {
      ...
      addParameter(
        "Name of Cinematographer or Film Director",
        matchingName ? `inspired by ${matchingName}` : segment,
    ...
    if (isFilmStock(segment)) {
      ...
      addParameter("Film Stock", matchingStock || segment, FILM_STOCK);
    }
...
const renderHighlightedPrompt = (
  ...
  segments.forEach((segment, index) => {
    ...
    const isPhotographyType =
      ...
      !segment.toLowerCase().includes("aesthetic") &&
      index === 1; // Always highlight the second segment if it contains "photography" Type
    ...
    const isShotType = SHOT_TYPES.some((shot) => {
      return isMatchingShotType(segment, shot);
    });
    ...
    // Always highlight Portrait Photography segments
    const isPortraitPhotography =
      segment.trim().toLowerCase() === "portrait photography";
...
// Clean up the prompt by removing duplicate parameters and ensuring proper format
const cleanPrompt = (text: string): string => {
  if (!text) return "";

  // First, strip out all AR and style raw parameters completely
  let cleanedText = text.trim();

  // Clean all parameter patterns at the end
  cleanedText = cleanedText.replace(
    /\s*--ar\s+\d+:\d+\s*--style\s+raw\s*$/g,
    ""
  );
  cleanedText = cleanedText.replace(/\s*--style\s+raw\s*$/g, "");
  cleanedText = cleanedText.replace(/\s*--ar\s+\d+:\d+\s*$/g, "");

  // Clean any internal duplicate parameters too
  cleanedText = cleanedText.replace(
    /\s*--ar\s+\d+:\d+\s*--style\s+raw\s*,/g,
    ","
  );
  ...
  // Fix multiple commas and spacing issues
  cleanedText = cleanedText.replace(/,\s*,+/g, ","); // Fix multiple commas
  cleanedText = cleanedText.replace(/\s{2,}/g, " "); // Fix multiple spaces
  cleanedText = cleanedText.replace(/,\s*$/g, ""); // Remove trailing comma
...
// Ensures prompt follows the correct format: starts with "Cinematic" and ends with "--ar 16:9 --style raw"
const ensureProperPromptFormat = (text: string): string => {
  if (!text) return "";

  // Clean the input text first
  let processedText = cleanPrompt(text);
  ...
  if (!processedText.startsWith("Cinematic")) {
    ...
    // If it starts with "Cinematic" but not "Cinematic," add the comma
    processedText =
      "Cinematic, " + processedText.substring("Cinematic".length).trim();
  }

  // Add required suffix "--ar 16:9 --style raw"
  ...
  const analyzePhoto = async () => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error(
          "Server error: Expected JSON response but got " + contentType
        );
      }
      ...
      let rawPrompt = data.generatedPrompt || "";

      // First clean the prompt to remove any duplicates
      let cleanedPrompt = cleanPrompt(rawPrompt);

      // Then format it properly
      // let formattedPrompt = ensureProperPromptFormat(cleanedPrompt);
      setGeneratedPrompt(cleanedPrompt);
      setEditablePrompt(cleanedPrompt);
      setEditableParameters(parsePrompt(cleanedPrompt));
  ...
  return (
    <div className="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold mb-4">Image Prompt Generator</h1>
        <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
          Generate powerful prompts from text or images to create stunning
          visuals. Transform your description or photo into an optimized prompt
      ...
      <Card className="w-full">
        <CardContent className="p-6 space-y-4">
            ...
            {generatedPrompt && !selectedImage && (
              <div ref={generatedPromptRef} className="space-y-4 mt-4">
                ...
                <Textarea
                  ...
                  onChange={(e) => {
                    // Set the raw value during typing
                    const originalValue = e.target.value;
                    setEditablePrompt(originalValue);

                    // Also update the generatedPrompt and editableParameters to reflect immediate changes
                    setGeneratedPrompt(originalValue);
                    setEditableParameters(parsePrompt(originalValue));
          ...
          <div className="w-full">
            <h2 className="text-2xl font-bold mb-4">IMAGE TO PROMPT1</h2>
